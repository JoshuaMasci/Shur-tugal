 ...
 */
package Shurtugal.client.model.anim;

import Shurtugal.client.model.DragonModel;
import Shurtugal.client.model.ModelPart;
...
import Shurtugal.common.util.Spline;
...
public class DragonAnimator
{
	...
	private float[] xGroundWalk2 =
	{ 0, 0, 0, 0 };

	// Y rotation angles for ground, thigh only
	private float[] yGroundStand =
	{ -0.25f, 0.25f };
	private float[] yGroundSit =
	{ 0.1f, 0.35f };
	private float[] yGroundWalk =
	{ -0.1f, 0.1f };
	...
	// 1st dim - front, hind
	// 2nd dim - thigh, crus, foot, toe
	private float[][] xAirAll =
	{
	{ 0, 0, 0, 0 },
	{ 0, 0, 0, 0 } };

	// Y rotation angles for air, thigh only
	private float[] yAirAll =
	{ -0.1f, 0.1f };

	// helpers
	private ThirdPersonCameraAccessor tpca = new ThirdPersonCameraAccessor();

	public DragonAnimator(EntityBaseDragon EntityBaseDragon)
	{
		this.entity = EntityBaseDragon;
	}
	...
	public void animate(DragonModel model)
	{
		...
		// update flags
		model.back.isHidden = entity.isSaddled() != 0;
	...
	public void update()
	{
		...
		boolean speedFlag = speedEnt > speedMax || onGround;
	...
	protected void animHeadAndNeck(DragonModel model)
	{
		...
		float health = entity.getDragonHealth2() / (float) entity.getMaxHealth();
		float neckSize;
	...
	protected void animWings(DragonModel model)
	{
		...
		float[] yFold = new float[]
		{ 2.7f, 2.8f, 2.9f, 3.0f };
		float[] yUnfold = new float[]
		{ 0.1f, 0.9f, 1.7f, 2.5f };
		...
		{
			model.wingFinger[i].rotateAngleX = rotX += 0.005f; // reduce
																// Z-fighting
			model.wingFinger[i].rotateAngleY = MathX.slerp(yUnfold[i], yFold[i] + rotYOfs * rotYMulti, ground);
			rotYMulti -= 0.2f;
		}
	}
	...
	protected void animTail(DragonModel model)
	{
		...
		for (int i = 0; i < model.tailProxy.length; i++)
		{
			...
			model.tail.rotateAngleX += MathX.toRadians(MathX.normDeg(pitchOfs));
			model.tail.rotateAngleX -= (1 - speed) * vertMulti * 2;
			model.tail.rotateAngleY += MathX.toRadians(MathX.normDeg(yawOfs + 180));
	...
	private void splineArrays(float x, boolean shift, float[] result, float[]... knots)
	{
		...
		// if (true) {
		// if (shift) {
		// System.arraycopy(knots[(int) (x + 1) % knots.length], 0, result, 0,
		// knots.length);
		// } else {
		// System.arraycopy(knots[(int) x % knots.length], 0, result, 0,
		// knots.length);
		// }
		// return;
		// }
